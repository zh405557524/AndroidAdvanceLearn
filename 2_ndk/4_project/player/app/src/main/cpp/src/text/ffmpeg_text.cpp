...
void FFMpegText::decodeAudio(std::string input, std::string output) {
    avformat_network_init();
//    总上下文
    AVFormatContext *formatContext = avformat_alloc_context();
    //打开音频文件
    if (avformat_open_input(&formatContext, input.c_str(), NULL, NULL) != 0) {
        LOGI("%s", "无法打开音频文件");
        return;
    }
...
//    初始化转换器其他的默认参数
    ...
    FILE *fp_pcm = fopen(output.c_str(), "wb");
    //读取包  压缩数据
